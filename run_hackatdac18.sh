#!/bin/bash
# Usage: ./run_hackatdac18.sh [--no-eager] [--no-sliding] [--both-off]
...
MILESTONE_DIR="milestones/hackatdac18"
LOG_DIR="logs/hackatdac18/${ABLATION_TAG}"
PROPERTIES_SV="designs/benchmarks/hackatdac18/properties.sv"
...
for milestone_file in "$MILESTONE_DIR"/*.json; do
    ...
    /usr/bin/time -v \
        timeout "$TIMEOUT_SEC" python3 -m main 30 hackdac18.F --sv \
        --auto-plan \
